...
| NXT-OBS-033 | 2026-07-29 | Windows başlangıç dayanıklılığı | Kullanıcı onayıyla `BTB Mobile BFF` logon task’ı açık `pilot + DisableNotificationDelivery` profiliyle kaydedildi. Task `Ready`, manuel tetik sonrası sonuç `0`; tek 4004 listener ile local/public health ve authenticated dashboard HTTP 200 geçti. Task argümanında secret yoktur. Gerçek Windows reboot sonrasında otomatik kalkış observation’ı gerekir. | HIGH | READY |
...
| NXT-OBS-066 | 2026-08-09 | Notification teslimi | Standalone producer cutover’ı yerel olarak hazırlandı: BFF mevcut `X-BTB-Notification-Key` yolunu korurken SAP SM59 için HTTPS Basic adaptörü ekledi; sabit kullanıcı `btb-notifier`, parola raw producer key ve runtime’da yalnız SHA-256 digest sözleşmesi test edildi. ABAP yerel kaynağı eski sabit IAS-korumalı CAP URL’sinden `ZBET_NOTIFICATION_API` destination + `/v1/notifications` + tür/idempotency sözleşmesine geçirildi. BFF `65/65`, production build ve ABAP local check geçti. Aktif runtime hâlâ `notificationService=unconfigured`; producer digest/ledger etkinleştirmesi, SM59 tanımı, SAP DEV aktivasyonu ve fiziksel gerçek FCM kanıtı ayrı onaylı dış kapılardır. | HIGH | DEFERRED |
| NXT-OBS-067 | 2026-08-09 | Yeniden başlatma dayanıklılığı | Reboot sonrası 502 kök nedenleri için startup zinciri açık auth/degraded profilini taşır hale getirildi. Kullanıcı onayıyla aktif BFF `pilot + DisableNotificationDelivery` modunda kaldırıldı ve aynı komut Windows logon task’ına kaydedildi. Task manuel çalıştırma sonucu `0`; local/public health ve authenticated dashboard HTTP 200. Notification teslimi bu process’te bilinçli kapalıdır. Gerçek reboot otomatik başlangıç kanıtı beklenir. | HIGH | READY |
