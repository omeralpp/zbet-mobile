...
Durum: `BTB ROADMAP MILESTONE 2/11 — LOGO / BRAND ASSET · UYGULAMA TAMAM,
SAHİP FİZİKSEL DOĞRULAMASI BEKLİYOR`
...
tema, launcher ikonu ve açılış ekranı. Milestone 2 **kapanmadı**: marka her yüzeyde görünür
değiştiği için sahibin Xiaomi cihazındaki fiziksel doğrulaması gerekiyor.
Kontrol listesi `docs/CURRENT_HANDOFF.md` içindedir.
...
| NXT-OBS-101 | 2026-08-18 | Kanonik BTB marka varlığı | **KODDA ÇÖZÜLDÜ (2026-08-19).** Kare zemin raster'ın içindeydi (192x192, tam saydam piksel 0, köşeler `(0, 8, 27, α=221)`) ve kodda kapatılamıyordu. Üretilen Intelligence Noir master'ı doğrulandı (gerçek saydamlık, beyaz saçak yok, kimlik yerinde), deterministik olarak 1024² saydam tuvale normalize edildi ve iki role türetildi: `btb-mark.png` (%92) ve `btb-adaptive-foreground.png` (%62 güvenli bölge). `expo prebuild` sonrası `ic_launcher_foreground` 0 saydam pikselden **%79** saydama, köşe alfası 221'den **0**'a geçti; bildirilen `#04101E` adaptive arka plan artık görünüyor. Açılış ekranındaki `borderRadius: 30` telafisi aynı değişiklikte kaldırıldı. `npm run check:brand` iki rol için de PASS ve `npm run check` kapısına eklendi. 18 dp dahil küçük boy siluet doğrulaması geçti. Fiziksel Xiaomi doğrulaması bekleniyor. | HIGH | READY |
...
| NXT-OBS-103 | 2026-08-19 | Marka varlığı fiziksel doğrulaması | Yeni marka launcher ikonu, açılış ekranı, Özet hero ve takım arması fallback'i olmak üzere dört yüzeyi birden değiştiriyor. Emülatörde release APK ile doğrulananlar: launcher ikonu daire maskede tam (kalkan kırpılmıyor, `#04101E` arka plan görünüyor), açılış ekranında kare/yuvarlatma/köşe kırpılması yok, Özet hero koyu ve açık temada kutusuz. **Gerçek Xiaomi launcher teması ve gerçek FCM bildirimi küçük ikonu yalnız fiziksel cihazda görülebilir**. Kontrol listesi `docs/CURRENT_HANDOFF.md` içindedir. Milestone 2 bu doğrulama olmadan kapatılamaz. | HIGH | OBSERVED |
