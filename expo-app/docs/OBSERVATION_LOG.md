...
| NXT-OBS-075 | 2026-08-11 | Bibi rehberi ayarları | Daha fazla ekranında Bibi rehberi durum satırı ile Bibi rehberini baştan başlat eylemi birlikte görünüyor. Kullanıcı yalnız yeniden başlat eyleminin kalmasını, durum satırının kaldırılmasını istedi. | LOW | OBSERVED |

| NXT-OBS-076 | 2026-08-11 | Genel UI düzeni ve bilgi hiyerarşisi | Kullanıcı tüm Mobile ekranlarının tasarım standartlarına uygun, nizami ve tutarlı biçimde yeniden düzenlenmesini istedi. Özellikle Super Karar Detayı'nda başlık/maske aralığı, maç özet kartı, Karar özeti metrik grid'i, karar anındaki saha, benzerlik-lig bağlamı, puan durumu tablosu ve alt aksiyon butonları arasında hizalama, spacing, tipografi ve bilgi yoğunluğu standardize edilmeli; diğer ekranlar da aynı ortak layout kurallarına taşınmalı. | MEDIUM | OBSERVED |

| NXT-OBS-077 | 2026-08-11 | Bibi hedef vurgusu ve kapsamlı rehber | Mevcut Bibi balonu açıklama yapılan bölümü yeterince belirgin vurgulamıyor; kullanıcı hangi kart/alanın anlatıldığını anında anlayamıyor. Rehber adımı aktif hedefi dim/outline/anchor overlay ile açıkça highlight etmeli. Ayrıca Özet, Canlı, Super, Toto, filtreler, metrik kartları, maç kartı aksiyonları, oran/baskı göstergeleri ve detay sayfalarındaki önemli fakat kendini açıklamayan bölümler kısa, bağlamsal ve ekran hedefli adımlarla anlatılmalı; rehber akışı küçük ekranı kapatmamalı ve tekrar başlatılabilir kalmalı. | MEDIUM | OBSERVED |

| NXT-OBS-078 | 2026-08-11 | Detay ekranı edge-swipe navigasyonu | Kullanıcı detay ekranlarında soldan sağa parmak hareketiyle geldiği ana/önceki ekrana dokunmatik kullanıma uygun, akıcı bir geçiş istiyor. Edge-swipe yalnız detay ekranında etkin olmalı; sistem/Android geri davranışıyla aynı navigation stack kullanılmalı, tab sırası ve mevcut buton yönleriyle çelişmemeli, yatay kart/list scroll hareketlerini engellememeli ve küçük ekranlarda güvenilir dokunma eşiği ile çalışmalı. | MEDIUM | OBSERVED |

| NXT-OBS-079 | 2026-08-11 | Opsiyonel açık tema | Mevcut dark tema varsayılan olarak korunacak; aynı ortak tasarım token'larından türetilen, kullanıcı tercihiyle açılıp kapanan bir light tema değerlendirilecek. Renk kontrastı, grafik/başarı-kayıp renk anlamları, Bibi ve widget uyumu ile Android sistem teması doğrulanmalı. Ayrı görsel faz olarak planlanması mevcut cutover kapsamını kontrollü tutar. | LOW | OBSERVED |
