...
| NXT-OBS-108 | 2026-08-21 | Toto program listesinde ikramiye göstergesi | Toto Programları listesinde gerçek ikramiye kazandıran programlar diğer sonuçlanmış programlardan küçük ama açık bir işaretle ayrılmalı. Tercih edilen çözüm, tüm kartı yeşil çerçeveye almak yerine durum alanı yakınında kompakt yeşil bir `₺ İkramiye` rozeti kullanmaktır; bu çözüm kart hiyerarşisini bozmaz, aktif/başarılı durum renkleriyle daha az çakışır ve yalnız renge dayanmaz. Doğrulanmış gerçek tutar ve para birimi mevcutsa rozet veya erişilebilirlik metni formatlı tutarı da taşıyabilir; para birimi TRY ise `$` değil `₺` kullanılmalıdır. Gösterge yalnız Toto-native, güvenilir ve pozitif **gerçek ikramiye/ödeme** alanından üretilmeli; `ana isabet`, `kapsama isabet`, kolon maliyeti veya detay ekranındaki `Teorik ikramiye` tek başına kazanç kanıtı sayılmamalıdır. `NXT-OBS-090` kapsamındaki nullable tutar/para birimi kaynağı doğrulanmadan Mobile sahte kazanan işareti üretmemeli; veri yoksa kart bugünkü görünümünü korumalıdır. Uzun tutar, farklı para birimi, aynı kartta `SONUÇLANDI` rozeti, dar ekran, açık/koyu tema, renk körlüğü ve ekran okuyucu açıklaması doğrulanmalıdır. **Sınıflandırma (2026-08-21):** Mevcut `TotoProgram` DTO'sunda yalnız `theoreticalPrize`/açıklama var; doğrulanmış gerçek ödeme, tutar ve para birimi alanı yok. Teorik tutardan kazanan türetilmedi. Toto-native kaynak alanı `NXT-OBS-090` sahipliğinde gelene kadar ertelendi. | MEDIUM | DEFERRED |
...
| NXT-OBS-120 | 2026-08-22 | Ana sekme sürükleme geçişi `PanResponder` (JS thread) ile yapılıyor — SofaScore karşılaştırması | Kullanıcının paylaştığı 30 sn ekran kaydı SofaScore'u (0–15s) ve BTB Mobile'ı (16–30s) art arda gösteriyor; ~10 örnek kare (Claude Browser ile `video.currentTime` seek + screenshot) incelendi. SofaScore'da `Events/Teams/Competitions/Athletes` üst sekmeleri arasında parmakla 1:1 sürüklenen, yarı-geçişte iki ekranın da aynı anda göründüğü yatay sayfa geçişi var (9.0–9.4s). Kullanıcı sahada BTB'de bunun eşleniği olan **ana sekme sürükleme** (Özet/Canlı/Super/Toto/Daha Fazla) hareketinin kötü hissettirdiğini doğruladı. Kök neden koda bakılarak bulundu: `src/components/Screen.tsx` bu sürüklemeyi React Native'in eski `PanResponder` + `Animated` API'siyle uyguluyor (`onPanResponderMove` her parmak hareketinde JS thread'e köprüden geçip `swipeX.setValue(...)` çağırıyor). Bu, native thread/worklet üzerinde çalışan `react-native-gesture-handler` + `react-native-reanimated`'ın aksine, JS thread meşgulken (liste render'ı, veri işleme) parmak takibinin gecikmesine/atlamasına açık — SofaScore gibi olgun uygulamaların akıcı sürüklemesi tipik olarak worklet tabanlı gesture kullanır. Proje `package.json`'da bu iki kütüphaneyi doğrudan bağımlılık olarak listemiyor, ama `package-lock.json`'da (muhtemelen `expo-router`/`react-native-screens` üzerinden transitive) zaten mevcutlar — yani baştan kurulum gerekmeden `Screen.tsx`'in sürükleme kısmı `Gesture.Pan()` + `useAnimatedStyle`'a taşınabilir. Alt sekme geçişinin geri kalanı (`app/(tabs)/_layout.tsx`: `sceneStyleInterpolator` + `mainTabTransitionSpec`, 280ms) ve edge-swipe-back (`NXT-OBS-078`, READY) aynı `Screen.tsx` gesture kökünü paylaşıyor, dolayısıyla düzeltme oraya da dokunur. Henüz kod değişikliği yapılmadı (observation-only); cihaz üstü FPS/profiler kanıtı da alınmadı. **`mobile cutover start` batch değerlendirmesi (2026-08-22): blocker olarak bırakıldı, uygulanmadı.** `react-native-gesture-handler`/`react-native-reanimated` yalnız transitive (muhtemelen `react-native-screens` üzerinden) mevcut; ne `babel.config.js` (dosya hiç yok) ne `GestureHandlerRootView` ne de worklet plugin'i kurulu. `Screen.tsx`'i worklet tabanlı gesture'a taşımak bu üçünü sıfırdan eklemeyi gerektirir — bu da her dosyanın derlenme şeklini değiştiren, uygulama genelinde paylaşılan bir gesture kökünü (hem ana sekme sürüklemesi hem edge-swipe-back) etkileyen, geri dönüşü zor bir altyapı kararı. Prosedürün kendi kuralı gereği ("kapsam genişlemesi veya geri döndürmesi zor işlem varsayımla uygulanmaz") bu tek batch'e sığdırılmadı; ayrı bir onay ve fiziksel cihaz doğrulama turu gerektiriyor. **2026-08-23 kapsam belirleme (kod uygulanmadı):** `react-native-gesture-handler@3.1.0` ve `react-native-reanimated@4.5.3` `node_modules`'ta yalnız transitive (başka bir paketin opsiyonel peer bağımlılığı) olarak kurulu; kodda hiç import edilmiyor. Proje kökünde hiçbir `babel.config.js`/`.babelrc` yok (Reanimated babel plugin'i kayıt gerektirir) ve `GestureHandlerRootView` kurulu değil. `android/gradle.properties`'te `newArchEnabled=true` zaten aktif, yani Reanimated 4'ün New Architecture önkoşulu zaten karşılanıyor — bu geçişin en büyük olağan riski burada yok. Sürükleme karar mantığı (`src/navigation/tab-swipe.ts`: `shouldActivateTabSwipe`, `tabSwipeTranslation`, `shouldCommitTabSwipe`) zaten saf ve gesture kütüphanesinden bağımsız; bir `Gesture.Pan()` yeniden yazımında testleriyle birlikte değişmeden kalabilir. Sahip bu turda ilerletmemeyi tercih etti (aciliyet yok); ayrı onay ve fiziksel cihaz doğrulama turu gerektiren blocker olarak açık kalıyor. **Çözüldü (2026-08-23, `mobile cutover start` batch):** Expo SDK 57 ile uyumlu doğrudan bağımlılıklar `react-native-gesture-handler ~2.32.0`, `react-native-reanimated 4.5.1` ve `react-native-worklets 0.10.1` eklendi; uygulama kökü `GestureHandlerRootView` ile sarıldı. `Screen.tsx` ana/yerel sekme ve edge-back sürüklemesini JS-thread `PanResponder` yerine `Gesture.Pan()` + Reanimated shared value/worklet zincirine taşıyor; navigation yalnız commit anında JS'e planlanıyor. Saf eşik/yön yardımcıları korunup edge testleri eklendi. Mobile type/lint/test/Doctor-bounded, Android production bundle, staged arm64 debug compile ve Android 15 x86_64 release cihaz smoke'u geçti: `Özet → Canlı`, yerel `Canlı → Fikstür`, sınırda `→ Super`, ters yön ve detay edge-back doğru çalıştı; fatal/Reanimated/GestureHandler hatası görülmedi. | HIGH | RESOLVED |
