...
- The Cordova wrapper uses the official Fiori renderer API for a mobile shell focus mode: the navy header returns on route changes, hides after 3 seconds of inactivity to release content space, and can be restored from the slim top-edge handle. Open dialogs and the football quick menu postpone hiding.
